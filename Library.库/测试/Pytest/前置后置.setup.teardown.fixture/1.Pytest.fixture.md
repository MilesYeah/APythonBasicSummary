...
通常，在项目中，我们会通过 conftest.py 和 @pytest.fixture 结合使用实现全局的前置应用。
比如项目的全局登录，模块的全局处理

某个或某几个用例开始前和结束都要进行操作处理
1. options 自定义用例开始前初始化和结束后结束操作
2. yield 之前定义每个 case 开始前的动作
3. yield 之后定义每个 case 结束后的动作
     ```py
     @pytest.fixture()       # 将 options 转化为测试组件
     def options(self):
         cmd_preactions      # 每个 case 开始前的动作
         yield
         cmd_postactions     # 每个 case 结束后的动作

     def test_case_1(self, options):     # 使用 options
         # case_actions 之前会跑 options 中 yield 之前的内容
         case_actions
         # case_actions 之后会跑 options 中 yield 之后的内容
...
1. `scope`: 
   ...
   4. `module`： 
      ...
      2. 执行顺序：
         ...
         2. fixture
         3. tests
      3. 每一个 .py 文件调用一次，该文件内又有多个function和class
   5. `package` / `session`： 
      1. 表示装饰某个 package/session ，在 package/session 的前后执行，在每个 session 中只执行一次
      3. 是多个文件调用一次，可以跨 .py 文件调用，每个 .py 文件就是 module
      2. fixture(scope='session') 函数写在 conftest.py 文件中
      4. conftest.py 文件文件名固定，放在工程根目录则是全局使用，放在某个package下，则只针对该package下的py文件有效
2. `params`: 参数化,
   1. 支持
      1. 列表 []
      2. 元组 () 
      ...
      4. 字典元组 ({}, {}, {}, ...)
   2. params 是参数名 是有s的
   3. 返回 request.param 这是属性名，是不带s的
3. `autouse：` 
   1. 自动执行，默认 False
   2. 如果加上他，就没必要在每个case的参数中添加fixture的名字，所有的case都会使用该fixture
4. `ids`: 
   1. 当前使用 params 参数化时,给每个值设定一个变量名，意义不大
5. `name`: 
...
1. scope=session
2. fixture为session级别是可以跨.py模块调用的,也就是当我们有多个.py文件的用例时候，如果多个用例只需调用一次fixture，那就可以设置为scope="session"，并且写到conftest.py文件里
3. conftest.py文件名称是固定的，pytest会自动识别该文件。放到工程的根目录下，就可以全局调用了，如果放到某个package包下，那就只在该package内有效
4. 如果想同时运行test_fixture11.py和test_fixture12.py，在cmd执行 `> pytest -s test_fixture11.py test_fixture12.py`
...
测试类前面添加：@pytest.mark.usefixtures('fixtureName') 其中 fixtureName 为 fixture 名。
1. 作用域为 class 时表示整个类中都只会执行一次。
1. 作用域为module时表示整个模块中都只会执行一次。
2. 作用域为package时表示整个包都只会执行一次。
3. 在N个用例中指定某一个用例执行
