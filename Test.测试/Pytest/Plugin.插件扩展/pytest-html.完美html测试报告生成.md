...
* `--html=report_filename.html` 
  * 添加该参数可生成名为 `report_filename.html` 的测试报告，测试报告分两部分，
    * assets 目录包含样式文件 如 css
    * report_filename.html 即测试报告
* `--self-contained-html` 
  * 该参数会将样式文件直接集成到 html 报告中。
...
port.html"
==================================== test session starts ====================================
platform win32 -- Python 3.8.5, pytest-6.0.1, py-1.10.0, pluggy-0.13.1 -- c:\e
